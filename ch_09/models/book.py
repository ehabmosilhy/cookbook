...
class LibraryBook(models.Model):
    ...
    state = fields.Boolean('Available')
    ...
    def get_min_pages(self):
        m=self.env['ir.config_parameter'].search([('key','=','pages')])
        return m
    ...
    @api.constrains('date_release', 'pages')
    def check_release_date(self):
        min_pages=self.get_min_pages()
        for this_record in self:
            if this_record.date_release > fields.Date.today():
                raise models.ValidationError('Release date must be in the past')
            if this_record.pages <int(min_pages.value):
                raise models.ValidationError('Pages must be greater than or equal to '.format(min_pages.value))
